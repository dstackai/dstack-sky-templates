...
Currently, the scope of templates is limited to `dstack` UI:
  ...
  - Each template has `type` set to `ui-template`, `id` (an identifier unique inside the repo), `title` (human-readable name of the template to be shown in the list of templates), `parameters`, and `configuration`.
