...
Currently, the scope of templates is limited to `dstack` UI:
  ...
  - Each template has `type` set to `ui-template`, `id` (an identifier unique inside the repo), `title` (human-readable name of the template to be shown in the list of templates), `parameters`, and `template` (the dstack run configuration).
  ...
  - The UI prompts the user to configure the parameters the UI supports and applies them to `template`.
